...
surface.middleware = function(app) {
  ...
  if (conf.options) {
    var rs = app.router.routes;
    var optionsRoutes = {};

    for (var i = rs.length - 1; i >= 0; i--) {
      optionsRoutes[rs[i].path] = optionsRoutes[rs[i].path] ? optionsRoutes[rs[i].path].concat(rs[i].methods) : rs[i].methods;
    };
    for (var path in optionsRoutes) {
      var methods = {};
      for (var i = optionsRoutes[path].length - 1; i >= 0; i--) {
        methods[optionsRoutes[path][i]] = true;
      };
      methods = Object.keys(methods);
      methods.push('OPTIONS');
      app.register(path, ['OPTIONS'], function *(next) {
        this.set('Access-Control-Allow-Methods', methods.sort().join(','));
        this.set('Access-Control-Allow-Headers', conf.options);
        this.body = '';
        yield next;
      });
      debug('Route Added:  %s %s - %s', 'OPTIONS', path, methods);
    }
  }
